...
def _get_manager_channel(guild: discord.Guild):
    return discord.utils.get(guild.text_channels, name=group_manager_channel_name)

async def _post_manager_poll(guild: discord.Guild, requester: discord.Member, study_topic: str, invitees, study_channel: discord.TextChannel):
    manager_channel = _get_manager_channel(guild)
    if not manager_channel:
        return None

    invitee_names = ", ".join(member.display_name for member in invitees) if invitees else "No invitees listed"
    embed = discord.Embed(
        title="New Study Group Pending Approval",
        description=f"**Subject:** {study_topic}",
        color=discord.Color.gold(),
    )
    embed.add_field(name="Creator", value=f"{requester.mention} ({requester.display_name})", inline=False)
    embed.add_field(name="Invited Members", value=invitee_names, inline=False)
    embed.add_field(name="Channel", value=f"#{study_channel.name}", inline=False)
    embed.set_footer(text="React with 👍 to approve or 👎 to remove the group.")

    message = await manager_channel.send(embed=embed)
    pending_group_approvals[message.id] = {
        "guild_id": guild.id,
        "study_channel_id": study_channel.id,
        "study_topic": study_topic,
        "requester_id": requester.id,
        "manager_channel_id": manager_channel.id,
    }
    try:
...
async def _handle_group_disapproval(guild: discord.Guild, channel: discord.TextChannel, study_topic: str, admin_member: discord.Member):
    if not channel:
        return
    members_to_notify = [member for member in channel.members if not member.bot]
    notification = (
        f"The study group for **{study_topic}** was removed by {admin_member.display_name}. "
        "Please reach out to the admins if you have questions."
    )
    for member in members_to_notify:
        try:
            ...
            continue
    # Remove outstanding invite polls tied to this channel
    for message_id, invite in list(group_invites.items()):
        if invite.get("channel_id") == channel.id:
            group_invites.pop(message_id, None)
    try:
        await channel.delete(reason=f"Disapproved by {admin_member.display_name}")
    except discord.Forbidden:
        pass
    except discord.HTTPException:
        pass
...
async def _process_manager_poll_reaction(payload: discord.RawReactionActionEvent, approval_metadata: dict):
    guild = bot.get_guild(approval_metadata["guild_id"])
    if not guild:
        pending_group_approvals.pop(payload.message_id, None)
        return

    admin_member = guild.get_member(payload.user_id)
    if not admin_member:
        return
    ...
    study_channel = guild.get_channel(approval_metadata["study_channel_id"])
    requester = guild.get_member(approval_metadata["requester_id"])
    study_topic = approval_metadata["study_topic"]

    # Ensure we only handle one decision
    pending_group_approvals.pop(payload.message_id, None)

    if emoji == "👍":
        if study_channel:
            try:
                await study_channel.send(f"Approved by {admin_member.display_name}. Have a great study session!")
            except discord.Forbidden:
                pass
            except discord.HTTPException:
                pass
        if requester:
            try:
                dm = requester.dm_channel or await requester.create_dm()
                await dm.send(f"Your study group for **{study_topic}** was approved by {admin_member.display_name}.")
            except discord.Forbidden:
                pass
        return

    # emoji == "👎"
    await _handle_group_disapproval(guild, study_channel, study_topic, admin_member)
    if requester:
        try:
            dm = requester.dm_channel or await requester.create_dm()
            await dm.send(f"Your study group for **{study_topic}** was removed by {admin_member.display_name}.")
        except discord.Forbidden:
            pass
...
async def group_creation(ctx):
    ...
    manager_message = await _post_manager_poll(ctx.guild, ctx.author, study, invitees, study_channel)
    if not manager_message:
        await dm_channel.send(
            "Heads up: I couldn't find the #creation-of-groups-manager channel, so the admins were not notified automatically."
        )
    else:
        await dm_channel.send("Your study group is awaiting admin approval. You'll be notified if it's removed.")
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    ...
    approval = pending_group_approvals.get(payload.message_id)
    if approval:
        await _process_manager_poll_reaction(payload, approval)
